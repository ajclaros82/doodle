...
public class PollImportOnStartup {
	 ...
	 * <p>
	 * If the collection already contains documents (e.g. created before the `raw`
	 * field existed), we backfill `raw` by re-reading the sample dataset and
	 * updating matching documents by _id.
	 */
	@Bean
	ApplicationRunner importPollsOnStartup() {
		return (ApplicationArguments args) -> {
			...
			//
			// if (existing > 0) {
			// log.info("Polls import skipped: repository already has {} documents",
			// existing);
			// return;
			// }
